...
/* Add code BEGIN */
    if (stack_C_initialized == 1) return ERR_INITIALIZED;
    values = malloc(sizeof(void*) * 1000);
    if (values == NULL) return ERR_NO_MEM;
    capacity = 1000;
    size = 0;
...
/* Add code BEGIN */
    if (stack_C_initialized == 0) return ERR_NOT_INITALIZED;
    if (size == 0) return ERR_NO_ITEM;
    //seg fault
    if(item == NULL){
        return ERR_INVALID_ARG;
    }
    *item = values[size-1];
    ...
    if (capacity > 1000 && size / (double)capacity < 0.8)
    {
        ...
        values = realloc(values, capacity * sizeof(void*));
    }
...
/* Add code BEGIN */
    if (stack_C_initialized == 0) return ERR_NOT_INITALIZED;
    //if (item == NULL) return ERR_NO_ITEM;
    if (size == capacity)
    {
        capacity = (int)(size / 0.9);
        values = realloc(values, capacity * sizeof(void*));
